    ...
    DispatchQueue.global(qos: .userInitiated).async {
        ...
        let request = VNRecognizeTextRequest { (request, error) in
            ...
            guard let observations = request.results as? [VNRecognizedTextObservation] else {
                NSLog("ZapScan: recognizeText - no observations cast")
                result("")
                return
            }
            NSLog("ZapScan: recognizeText - found \(observations.count) text observations")
    ...
    DispatchQueue.global(qos: .userInitiated).async {
        ...
        let request = VNDetectBarcodesRequest { (request, error) in
            ...
            guard let observations = request.results as? [VNBarcodeObservation] else {
                NSLog("ZapScan: recognizeBarcode - no observations cast")
                result([])
                return
            }
            NSLog("ZapScan: recognizeBarcode - found \(observations.count) observations")
